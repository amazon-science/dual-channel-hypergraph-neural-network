...
class MLP(BaseGNN):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers, dropout):
        super(MLP, self).__init__(dropout, num_layers)
...
class GCN(BaseGNN):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers, dropout):
        ...
        for i in range(num_layers):
            ...
            second_channels = out_channels if i == num_layers - 1 else hidden_channels
            self.convs.append(
                GCNConv(first_channels, second_channels, normalize=False))


class HyperGCN(BaseHyperGNN):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers, dropout):
        super(HyperGCN, self).__init__(dropout, num_layers)
        for i in range(num_layers):
            first_channels = in_channels if i == 0 else hidden_channels
            second_channels = out_channels if i == num_layers - 1 else hidden_channels
            self.convs.append(HypergraphConv(first_channels, second_channels))
...
class MLPCatPredictor(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers, dropout):
        ...
        self.lins = torch.nn.ModuleList()
        # MLPCatPredictor concatenate two node embeddings with the same dimension, thus the input_channels are doubled.
...
class MLPDotPredictor(torch.nn.Module):
    ...
    def forward(self, x_i, x_j):
        for lin in self.lins:
            ...
            x_i, x_j = F.dropout(
                x_i, p=self.dropout, training=self.training
